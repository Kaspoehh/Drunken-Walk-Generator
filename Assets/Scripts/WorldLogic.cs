...
public class WorldLogic : MonoBehaviour
{
    ...
    /// <summary>
    /// Create the walkers called from button in the UI
    /// </summary>
    public void Generate()
    {
        ...
        for (int i = 0; i < _walkers.Count; i++)
        {
            StopCoroutine(_walkers[i].GetComponent<Walker>().Walk());
            Destroy(_walkers[i]);
        }
    ...
    /// <summary>
    /// Check if the max blocks to place in the world is reached
    /// </summary>
    ...
    /// <summary>
    /// Pick a biome and give this to the walker
    /// </summary>
    /// <param name="_mostExpectedBiome"></param>
    /// <param name="_secondMostExcpectedBiome"></param>
    /// <returns>_biomeType</returns>
    private Biomes PickBiome(int _mostExpectedBiome, int _secondMostExcpectedBiome) {
        
        Biomes _biomeType = new Biomes();
        
        int _randomNumber = Random.Range(0, _biomeCubes.Count + 10);
    ...
    /// <summary>
    /// Create the walkers in the world
    /// </summary>
    /// <returns></returns>
    IEnumerator GenerateWalkers()
    {
        ...
        for (int i = 0; i < _walkerAmount; i++)
        {
            ...
            if(_posY > 0)
                _walker_logic.Cube = PickBiome(4, 5);
            if(_posY > 83)
                _walker_logic.Cube = PickBiome(2, 3);
            if(_posY > 166)
                _walker_logic.Cube = PickBiome(0, 1);
            ...
            StartCoroutine(_walker_logic.Walk());
